...
# This app reads precomputed RDS objects to avoid unsupported browser-side parquet processing.
app_data_dir <- "data"
required_files <- c(
  "airbnb_data.rds",
  "all_words.rds",
...
missing_files <- required_files[!file.exists(file.path(app_data_dir, required_files))]
if (length(missing_files) > 0) {
  stop(
    paste0(
      "Missing precomputed data files in apps/airbnb-shinylive/data: ",
      paste(missing_files, collapse = ", "),
      ". Run scripts/prepare_shinylive_data.R first."
    )
  )
}

airbnb_data <- readRDS(file.path(app_data_dir, "airbnb_data.rds"))
all_words <- readRDS(file.path(app_data_dir, "all_words.rds"))
all_tokens <- readRDS(file.path(app_data_dir, "all_tokens.rds"))
all_bigrams <- readRDS(file.path(app_data_dir, "all_bigrams.rds"))
all_afinn <- readRDS(file.path(app_data_dir, "all_afinn.rds"))
all_bing <- readRDS(file.path(app_data_dir, "all_bing.rds"))
all_nrc <- readRDS(file.path(app_data_dir, "all_nrc.rds"))
